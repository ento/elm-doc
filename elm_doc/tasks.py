...
def create_tasks(project_path: str, output_dir: str, elm_make: str = None, exclude_modules: List[str] = [], mount_point: str = ''):
    ...
    for task in package_tasks.create_package_tasks(
            output_path, project_package, elm_make=elm_make, exclude_modules=exclude_modules, mount_point=mount_point):
        yield task

    for package in deps:
        for task in package_tasks.create_package_tasks(output_path, package, elm_make=elm_make, mount_point=mount_point):
            yield task
